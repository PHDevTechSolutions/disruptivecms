...
export async function extractTdsTemplateFields(
  templateUrl: string,
): Promise<string[]> {
  try {
...
function buildValueLookup(params: FillTdsPdfParams): Record<string, string> {
  ...
  for (const a of ["brand", "brandname"])
    lookup[a] = sanitizeForPdf(params.brand);
  for (const a of ["itemcode", "lititemcode", "ecoitemcode", "code"])
    lookup[a] = sanitizeForPdf(itemCode);
  for (const a of [
    "itemdescription",
    "description",
    "productname",
...
export async function fillTdsPdf(params: FillTdsPdfParams): Promise<string> {
  ...
  if (!templateRes.ok)
    throw new Error(`TDS template fetch failed: ${templateRes.status}`);
  ...
  if (!usedAcroForm) {
    ...
    const ROWS: { label: string; y: number }[] = [
      ...
      { label: "Wattage", y: 469.1 },
      { label: "Lumens Output", y: 453.1 },
      { label: "Color Temperature", y: 437.1 },
      { label: "CRI", y: 415.1 },
      { label: "Visual Angle", y: 399.1 },
      { label: "Light Source", y: 383.1 },
      { label: "Life Hours", y: 367.1 },
      { label: "Working Voltage", y: 329.1 },
      { label: "Power Factor", y: 313.1 },
      { label: "Surge Protection", y: 297.1 },
      { label: "Dimension", y: 259.1 },
      { label: "Materials", y: 243.1 },
      { label: "Lamp Body Color", y: 227.1 },
      { label: "Working Temperature", y: 211.1 },
      { label: "Weight", y: 195.1 },
      { label: "IK Rating", y: 179.1 },
    ...
    if (mainImageUrl)
      await drawImage(pdfDoc, page1, mainImageUrl, 34, 530, 155, 150);

    if (page2) {
      if (dimensionDrawingUrl)
        await drawImage(pdfDoc, page2, dimensionDrawingUrl, 34, 100, 255, 550);
      if (illuminanceLevelUrl)
        await drawImage(pdfDoc, page2, illuminanceLevelUrl, 310, 100, 255, 550);
    }
  }
  ...
  if (!familyMeta) {
    ...
    const snap = await getDocs(
      query(
        ...
        where("title", "==", product.productFamilyName),
      ),
    );
    const templateUrl: string = snap.docs[0]?.data()?.tdsTemplate ?? "";
    if (!templateUrl)
      throw new Error(`No TDS template for "${product.productFamilyName}"`);
